...
const mockInventoryData = [
  { Code: "06-0005-01", Class: "B", TargetSaleUnit_1: 70, SaleOut_มีค68: 43, SaleOut_เมย68: 41, SaleOut_พค68: 48, SaleOut_มิย68: 28 },
  { Code: "06-0003-01", Class: "B", TargetSaleUnit_1: 140, SaleOut_มีค68: 64, SaleOut_เมย68: 70, SaleOut_พค68: 71, SaleOut_มิย68: 65 },
  { Code: "06-0003-02", Class: "A", TargetSaleUnit_1: 120, SaleOut_มีค68: 72, SaleOut_เมย68: 76, SaleOut_พค68: 80, SaleOut_มิย68: 78 },
  { Code: "06-0003-03", Class: "C", TargetSaleUnit_1: 90, SaleOut_มีค68: 38, SaleOut_เมย68: 42, SaleOut_พค68: 39, SaleOut_มิย68: 40 },
  { Code: "06-0003-04", Class: "B", TargetSaleUnit_1: 150, SaleOut_มีค68: 81, SaleOut_เมย68: 79, SaleOut_พค68: 85, SaleOut_มิย68: 83 },
];

const months = ["มีค68", "เมย68", "พค68", "มิย68"];

// Define color palettes
const COLORS_BY_CODE = ['#640037', '#ff9800', '#00bcd4', '#4caf50', '#ff005c', '#8a2be2', '#00ced1', '#ffc0cb', '#7b68ee', '#f0e68c'];
...
const getBarChartData = (inventoryData) => {
  ...
  inventoryData.forEach((item) => {
    months.forEach((monthKey) => {
      ...
      dataByMonth[monthKey]["ยอดขายเป้าหมาย (หน่วย)"] += targetSale / months.length;
    });
  });

  return months.map((monthKey) => ({
    ...dataByMonth[monthKey],
    "ยอดขายเป้าหมาย (หน่วย)": Math.round(dataByMonth[monthKey]["ยอดขายเป้าหมาย (หน่วย)"]),
  }));
};
...
const getPieChartDataByCode = (inventoryData) => {
    const pieData = inventoryData.map((item) => {
        const totalSaleOut = months.reduce((sum, monthKey) => sum + (item[`SaleOut_${monthKey}`] || 0), 0);
        return {
...
const CustomPieTooltip = ({ active, payload }) => {
    if (active && payload && payload.length) {
        ...
        const chartData = data.name.startsWith('Class') ? pieChartDataByClass : pieChartDataByCode;
...
const AcAndFc = () => {
  ...
  const renderChart = () => {
    switch (chartType) {
      ...
      case 'pie-code':
        return (
            <div className="h-full flex flex-col items-center justify-center p-2">
                <h4 className="text-center font-bold text-sm text-[#640037] mb-2">สัดส่วนยอดขายรวมตามรหัสสินค้า</h4>
                <ResponsiveContainer width="100%" height="100%">
                    <PieChart margin={{ top: 0, right: 0, left: 0, bottom: 0 }}>
                        ...
                        >
                            {pieChartDataByCode.map((entry, index) => (
                                <Cell key={`code-cell-${index}`} fill={COLORS_BY_CODE[index % COLORS_BY_CODE.length]} />
                            ))}
                        </Pie>
                    </PieChart>
                </ResponsiveContainer>
      ...
      case 'pie-class':
        return (
            <div className="h-full flex flex-col items-center justify-center p-2">
                <h4 className="text-center font-bold text-sm text-[#640037] mb-2">สัดส่วนยอดขายรวมตาม Class (A, B, C)</h4>
                <ResponsiveContainer width="100%" height="100%">
                    <PieChart margin={{ top: 0, right: 0, left: 0, bottom: 0 }}>
                        <Legend 
                            iconType="circle" 
                        ...
                        >
                            {pieChartDataByClass.map((entry, index) => (
                                <Cell key={`class-cell-${index}`} fill={COLORS_BY_CLASS[entry.name.replace('Class ', '')]} />
                            ))}
                        </Pie>
                    </PieChart>
                </ResponsiveContainer>
  ...
  return (
    <div className="h-[450px] w-full flex flex-col"> {/* กำหนดความสูงให้ AcAndFc */}
      <div className="flex justify-center space-x-2 mb-4">
        <button
          onClick={() => setChartType('bar')}
          className={`px-4 py-2 text-sm rounded-lg transition-colors duration-200 ${
            chartType === 'bar' ? 'bg-[#640037] text-white shadow' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'
          }`}
        >
        ...
        <button
          ...
          className={`px-4 py-2 text-sm rounded-lg transition-colors duration-200 ${
            chartType === 'pie-code' ? 'bg-[#640037] text-white shadow' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'
          }`}
        >
        ...
        <button
          ...
          className={`px-4 py-2 text-sm rounded-lg transition-colors duration-200 ${
            chartType === 'pie-class' ? 'bg-[#640037] text-white shadow' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'
          }`}
        >
      ...
      <div className="flex-grow"> {/* ทำให้ส่วนกราฟขยายเต็มพื้นที่ที่เหลือ */}
        {renderChart()}
      </div>

      <p className="text-xs text-gray-500 text-right mt-4 ">
        {chartType === 'bar' && '*Target Sale คือค่าประมาณการเฉลี่ยรายเดือน'}
        {(chartType === 'pie-code' || chartType === 'pie-class') && '*ข้อมูลแสดงสัดส่วนยอดขายรวม (Sale Out) ตั้งแต่ มี.ค.68 ถึง มิ.ย.68'}
      </p>
    </div>
  );
};
