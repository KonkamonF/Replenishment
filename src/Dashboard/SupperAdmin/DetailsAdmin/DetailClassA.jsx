  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 z-40 flex justify-end">
      <div className="bg-white w-full h-full p-6 shadow-2xl z-50 overflow-y-auto">
        ...
        {isLoading ? (
          <p className="text-center text-gray-500 py-10 animate-pulse">กำลังโหลดข้อมูล...</p>
        ) : error ? (
          <p className="text-center text-red-500 py-10">โหลดข้อมูลไม่สำเร็จ: {error}</p>
        ) : (
          <>
            <table className="min-w-full border-collapse border-2 border-gray-200">
              <thead className="bg-[#640037] text-white">
                <tr>
                  ...
                  <th className="p-3">ManualClass</th>
              ...
              <tbody>
                {products.map((p, i) => (
                  <tr key={i} className="border-b hover:bg-amber-50 transition">
                    ...
                    <td className="p-3">{p.manualClass}</td>
                    <td className="p-3">{p.autoClass}</td>
